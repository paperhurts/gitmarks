...
**Known Playwright limitation:** `serviceWorker.evaluate()` in Playwright 1.x runs in an execution context isolated from the extension's `background.ts` module scope. Chrome extension events (bookmarks, alarms, runtime messages, storage changes) dispatched from `evaluate()` don't reach the module's registered listeners. The e2e sync tests work around this by inlining the equivalent algorithm into the evaluate context where the mocked `fetch` is available; the listener-debounce-flush dispatch path is covered by the unit tests, and the production wiring is covered by the manual smoke test.
